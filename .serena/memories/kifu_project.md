...
### 10. Go 1.26+ WASM 棋譜記録・画像認識ライブラリ (wasm-kifu) の新規作成と統合設計
- **ライブラリ設計**:
  完全なクライアントサイドで動作する、UIフリーな棋譜記録・画像認識ライブラリ `wasm-kifu/` を新規に構築した。
  - **Go WASM レイヤー**: `rooklift/sgf` をラップしたコアゲームエンジンによるルール判定、コウ・自殺手のチェック、SGFパース・シリアライズ、および `pkg/vision` による画像認識交点データからの新着手・アゲハマ自動推定を担当。
  - **JavaScript レイヤー**: `board-detector.js` 内で OpenCV.js を用い、4隅のタップ座標から正面図への射影補正と、グレースケール輝度・HSV彩度平均による交点石（黒/白/空）の自動分類を担当。
- **`syscall/js` によるグローバル登録への移行**:
  - `go:wasmexport` (Go 1.24+) は `string` を戻り値に持つ関数をサポートしないため、Go 標準ライブラリの `syscall/js` を用いた従来型のグローバル関数登録 (`wasmNewGame`, `wasmExportSGF` 等) を採用。`select {}` でメインスレッドの稼働を維持しつつ、JS 側で JSON/SGF 文字列のやり取りを可能にした。
- **Vite を用いた相対パスによる直接インポート統合 (コピーなし)**:
  - `kifu/frontend` にファイルをコピーせず `wasm-kifu` 側のオリジナルの JS/WASM を直接再利用できるよう、`vite.config.js` で `@wasm-kifu` エエイリアスを追加し、`server.fs.allow` でプロジェクト外ディレクトリへのアクセスを許可した。
  - WASM ファイル自体は Vite の `?url` プレフィックスによるインポート (`import kifuWasmUrl from "@wasm-kifu/web/wasm/kifu.wasm?url"`) を活用することで、Vite がビルド時に自動でアセットとして公開ディレクトリにマッピング・解決するように設計した。
  - `vite-env.d.ts` にアンビエントモジュール宣言 (`declare module "@wasm-kifu/..."`) を追加することで、TypeScript 環境下でもファイルのコピーを行うことなく、インポートした JS クラスに対して 100% 型安全な自動補完と静的型チェックが効くように構成した。
