...
├── wasm-kifu/             # Go WASM 棋譜記録・画像認識ライブラリ (新規作成)
│   ├── cmd/wasm/          # WASM エントリーポイント
│   ├── pkg/               # コアロジック (engine, vision)
│   └── web/               # デモUI、JSブリッジ、OpenCV.js検出ロジック
...
### ⑨ Go WASM 棋譜記録・画像認識ライブラリ (wasm-kifu) と相対パスでの直接インポート統合
* クライアントサイドでの棋譜記録・SGF生成・カメラ画像認識を行うため、新規に `wasm-kifu/` ライブラリを構築した。
* コピーによるファイル重複と二重管理を防ぐため、`kifu/frontend` から**相対パスで直接インポートして使用する**インフラ構成とした。
* **Vite の設定 (`vite.config.js`)**:
  `@wasm-kifu` エイリアスを定義し、`server.fs.allow` でプロジェクト外にある `../wasm-kifu` ディレクトリのロードを許可している。
* **WASM アセットの解決**:
  `.wasm` ファイルは、コード側で `import kifuWasmUrl from "@wasm-kifu/web/wasm/kifu.wasm?url"` のように `?url` アセットインポートを使用して読み込み、Vite がビルド時に自動で静的ディレクトリに配置・解決する設計を採用した。
* **TypeScript 型定義の解決 (`vite-env.d.ts`)**:
  `vite-env.d.ts` 内に `@wasm-kifu/web/js/wasm-bridge.js` および `board-detector.js` の型定義モジュール (`declare module`) をアンビエント宣言し、ファイルのコピーなしでも TypeScript 側で 100% 安全な自動補完と型検証が効くように構成した。

### ⑩ 棋譜一覧のタイルグリッド（カード）化と OGP の扱い
* **OGP画像の有無フラグ (`has_ogp`)**:
  データベースの巨大な `ogp_image` バイナリをリストAPI (`FindAllByUser` / `FindAllPublicByUser`) でそのままロードしてシリアライズすると、ネットワーク帯域とサーバーメモリを著しく浪費するため、`(ogp_image IS NOT NULL AND octet_length(ogp_image) > 0) AS has_ogp` という表現を用いて、軽量なフラグだけをバインドして返す設計とした。
* **プライベート OGP の取得**:
  共有用の `/api/share/{token}/og-image` や公開用の `/api/u/{userId}/kifus/{kifuId}/og-image` は、非公開に設定された棋譜の画像をロードできない。オーナーが自分のプライベートな棋譜の OGP 画像をタイル一覧で安全に表示できるよう、`GET /api/kifus/{id}/og-image`（要認証・所有者検証付き）を新設した。
* **1:1 正方形のタイルカード**:
  `KifuList.svelte` にて、OGPプレビューを `aspect-ratio: 1 / 1` および `object-fit: cover` の CSS で完全に均等な正方形にトリミングして表示する。OGP が未生成の棋譜に関しては、Washi Clay テーマの和風デザインを崩さないよう、碁盤を模したデザインの SVG プレースホルダーをインラインでレンダリングする。
