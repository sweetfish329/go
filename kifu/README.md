...
バックエンドサーバー（Go）は以下の環境変数によって動作やセキュリティ設定をカスタマイズできます。

| 環境変数名 | 説明 | デフォルト値 / 設定例 |
| :--- | :--- | :--- |
| `PORT` | バックエンドサーバーが起動するポート番号。 | `8080` |
| `DB_PATH` | SQLiteデータベースファイルの保存先パス。 | `/data/kifu.db` |
| `JWT_SECRET` | JWT トークン署名・検証用のシークレットキー。本番環境ではランダムな長い文字列を設定してください。 | `kifu-secret-key-1234` |
| `ALLOWED_ORIGIN` | CORSでアクセスを許可する特定のオリジン。本番環境で特定オリジンに制限する場合に設定。 | 空（`*` すべて許可にフォールバック） |
| `ADMIN_USERNAME` | 管理画面にログインする管理者ユーザー名。設定必須。 | なし（未設定の場合、ログインリクエスト時にサーバーが停止します） |
| `ADMIN_PASSWORD` | 管理画面にログインする管理者パスワード。設定必須。 | なし（未設定の場合、ログインリクエスト時にサーバーが停止します） |
| `ENCRYPTION_KEY` | OAuthクライアントシークレットをDBに暗号化保存するためのAES鍵。`JWT_SECRET`とは別のランダム値を強く推奨。 | `kifu-default-encryption-key-for-oauth` |
| `COOKIE_SECURE` | セッションCookieに `Secure` 属性を付与するか。`true` にするとHTTPS通信時のみCookieを送信します。 | `false` |
